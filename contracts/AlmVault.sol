...
/**
 ...
 AlmVaults should be deployed by the AlmVaultFactory. 
 AlmVaults should not be used with tokens that charge transaction fees.  
 */
contract AlmVault is
    IAlmVault,
    ...
    Ownable
{
    ...
    /**
     @notice creates an instance of AlmVault based on the pool. allowToken parameters control whether the AlmVault allows one-sided or two-sided liquidity provision
     ...
     @param __owner Owner of the AlmVault
    ...
    constructor(
        ...
        uint32 _twapPeriod,
        uint256 _vaultIndex
    ) ERC20("ALM Vault Liquidity", UV3Math.computeIVsymbol(_vaultIndex)) { 
    ...
    /**
     ...
     @param deposit0 Amount of token0 transfered from sender to AlmVault
     @param deposit1 Amount of token0 transfered from sender to AlmVault
    ...
    /**
     @notice Redeems shares by sending out a percentage of the AlmVault's AUM - this percentage is equal to the percentage of total issued shares represented by the redeeemed shares.
    ...
    /**
     @notice Updates AlmVault's LP positions.
    ...
    /**
     @notice Collects and distributes fees accumulated in the AlmVault's LP positions. Everybody is allowed to pay for this transaction.
    ...
    function _distributeFees(uint256 fees0, uint256 fees1) internal {
        ...
        if (baseFee > 0) {
            ...
            uint256 baseFeeSplit = (affiliate == NULL_ADDRESS)
                ? PRECISION
                : IAlmVaultFactory(almVaultFactory).baseFeeSplit();
            address feeRecipient = IAlmVaultFactory(almVaultFactory).feeRecipient();
   ...
   /**
    ...
    /**
     @notice Calculates total quantity of token0 and token1 in both positions (and unused in the AlmVault)
     @return total0 Quantity of token0 in both positions (and unused in the AlmVault)
     @return total1 Quantity of token1 in both positions (and unused in the AlmVault)
     */
