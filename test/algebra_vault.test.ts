...
describe('Access Control Checks', () => {
  ...
  beforeEach('deploy contracts', async () => {
    ({ token0, token1, token2, factory, router, nft, oracle, algebraVaultFactory } =
    ...
    await expect(algebraVault.connect(wallet).setDepositMax(ethers.parseEther('100000'), ethers.parseEther('100000')))
  ...
  it('AlgebraVault', async () => {
    await expect(algebraVault.connect(alice).rebalance(-1800, 1800, -600, 0, 0)).to.be.revertedWithCustomError(
      algebraVault,
      'NotRebalancer',
    );
    await expect(
    ...
    await expect(algebraVault.connect(alice).setTwapPeriod(1800)).to.be.revertedWithCustomError(
      algebraVault,
      'NotManager',
    );
  });
...
describe('Input Validation Checks', () => {
  ...
  beforeEach('deploy contracts', async () => {
    ...
    const tx = await algebraVaultFactory
      .connect(wallet)
      .createAlgebraVault(await token0.getAddress(), true, await token1.getAddress(), false);
  ...
  it('AlgebraVaultFactory - misc', async () => {
    ...
    await expect(
      algebraVaultFactoryFactory.deploy(NULL_ADDRESS, NULL_ADDRESS, NULL_ADDRESS, NULL_ADDRESS, 'VEL'),
  ...
  it('AlgebraVaultFactory - createAlgebraVault', async () => {
    ...
    await expect(
      algebraVaultFactory
        .connect(wallet)
        .createAlgebraVault(await token0.getAddress(), true, await token0.getAddress(), false),
    ).to.be.revertedWith(msg1);
    await expect(
    ...
    await expect(
      algebraVaultFactory
        .connect(wallet)
        .createAlgebraVault(await token0.getAddress(), false, await token1.getAddress(), false),
    ...
    await expect(
      algebraVaultFactory
        .connect(wallet)
        .createAlgebraVault(await token0.getAddress(), true, await token2.getAddress(), false),
  ...
  it('AlgebraVault - manual calls', async () => {
    ...
    await expect(algebraVaultFactory.deploy(NULL_ADDRESS, true, true, 'AV-TEST')).to.be.reverted;
  ...
  it('AlgebraVault - deposit', async () => {
    ...
    const tx1 = await algebraVaultFactory
      .connect(wallet)
      .createAlgebraVault(await token0.getAddress(), true, await token2.getAddress(), false);
    const tx2 = await algebraVaultFactory
      .connect(wallet)
      .createAlgebraVault(await token0.getAddress(), false, await token2.getAddress(), true);

    // check allowToken policy - first vault allows only token0
    let vaultKey = await algebraVaultFactory.genKey(
      await wallet.getAddress(),
      await token0.getAddress(),
    ...
    vaultKey = await algebraVaultFactory.genKey(
      await wallet.getAddress(),
      await token0.getAddress(),
      await token2.getAddress(),
      false,
    ...
    algebraVaultAddress = await algebraVaultFactory.getAlgebraVault(vaultKey);
    testVault = (await ethers.getContractAt('AlgebraVault', algebraVaultAddress)) as AlgebraVault;
    await expect(testVault.deposit(0, 0, await alice.getAddress())).to.be.revertedWithCustomError(
      testVault,
      'InvalidDeposit',
    );
    ...
    await expect(
      testVault.connect(alice).deposit(ethers.parseEther('4000'), 0, algebraVaultAddress),
    ).to.be.revertedWithCustomError(testVault, 'ZeroAddress');
  });
  ...
  it('AlgebraVault - withdraw', async () => {
    ...
    //check 'to' address
    await expect(
      algebraVault.connect(alice).withdraw(ethers.parseEther('4000'), NULL_ADDRESS),
    ).to.be.revertedWithCustomError(algebraVault, 'ZeroAddress');
    //check shares
    await expect(algebraVault.connect(alice).withdraw(0, await alice.getAddress())).to.be.revertedWithCustomError(
      algebraVault,
      'ZeroValue',
    );
  });
  ...
  it('AlgebraVault - rebalance', async () => {
    ...
    await expect(algebraVault.connect(wallet).rebalance(-1800, -1200, -1800, -1200, 0)).to.be.revertedWithCustomError(
      algebraVault,
      msg3,
    );
    await expect(algebraVault.connect(wallet).rebalance(1800, 1200, 60, 600, 0)).to.be.revertedWithCustomError(
      algebraVault,
      msg1,
    );
    await expect(algebraVault.connect(wallet).rebalance(-1800, -1200, -180, -600, 0)).to.be.revertedWithCustomError(
      algebraVault,
      msg2,
    );
    ...
    await expect(
      algebraVault.connect(wallet).rebalance(1800, 1000, 50, 550, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg1);
    await expect(
      algebraVault.connect(wallet).rebalance(-1800, 1000, 50, 550, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg1);
    await expect(
      algebraVault.connect(wallet).rebalance(-1000, 1800, 50, 550, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg1);

    await expect(
      algebraVault.connect(wallet).rebalance(-1800, 1200, -50, -550, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg2);
    await expect(
      algebraVault.connect(wallet).rebalance(-1800, 1200, -600, -500, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg2);
    await expect(
      algebraVault.connect(wallet).rebalance(-1800, 1200, -600, -550, rebalanceSwapAmount),
    ).to.be.revertedWithCustomError(algebraVault, msg2);
  });
  ...
  it('AlgebraVault - setTwapPeriod', async () => {
    ...
    await expect(algebraVault.connect(wallet).setTwapPeriod(0)).to.be.revertedWithCustomError(algebraVault, msg1);
