...
describe("Farming Integration", () => {
  ...
  describe("With deposits", () => {
    beforeEach("setup", async () => {
      ...
      await token0
        .connect(alice)
        .approve(algebraVault.address, largeTokenAmount);
      await token1
        .connect(alice)
        .approve(algebraVault.address, largeTokenAmount);
      ...
      await token1
        ...
        .approve(algebraVault.address, largeTokenAmount);
      // mint tokens to Bob
      await token0.mint(carol.address, largeTokenAmount);
      await token1.mint(carol.address, largeTokenAmount);
    });
    ...
    describe("Rewards Distribution", () => {
      ...
      beforeEach("Earn initial rewards", async () => {
        ...
        await algebraVault.approve(
          farmingRewardsDistributor.address,
          giantTokenAmount
        );
      ...
      it("Should handle rewards accrual when no users are staking", async () => {
        ...
        await farmingRewardsDistributor
          .connect(alice)
          .stake(aliceLpBalance, alice.address);
        ...
        const updatedRewardDataToken2 =
          await farmingRewardsDistributor.rewardData(token2.address);
      ...
      describe("Not first stakers", () => {
        beforeEach("Clean rewards", async () => {
          await farmingRewardsDistributor.stake(
            await algebraVault.balanceOf(wallet.address),
            wallet.address
          );
          ...
          await farmingRewardsDistributor.unstake(
            await farmingRewardsDistributor.totalBalance(wallet.address)
          );
          await farmingRewardsDistributor.getReward(wallet.address, [
            token1.address,
            token2.address,
          ]);
        });
        ...
        it("Should handle unstaking correctly and update reward claims", async () => {
          ...
          const [, startingAmounts] =
            await farmingRewardsDistributor.claimableRewards(alice.address);
          ...
          const newRewardsAccrued = finalAmounts[0]
            .sub(previousReward)
            .sub(startingAmounts[0])
            .add(claimedRewards);
          const previousTotalRewards = previousReward.mul(2); // Double the claimed amount as estimate

          const accruedRewardsDiff = newRewardsAccrued
            .mul(100)
            .div(previousTotalRewards);
        ...
        it("Handles getReward for specific reward tokens correctly", async () => {
          ...
          await algebraVault
            .connect(bob)
            .deposit(ethers.utils.parseEther("2000"), 0, bob.address);
          ...
          await algebraVault
            .connect(bob)
            .approve(farmingRewardsDistributor.address, bobLpBalance);
          await farmingRewardsDistributor
            .connect(bob)
            .stake(bobLpBalance, bob.address);
          ...
          const claimTx = await farmingRewardsDistributor
            .connect(bob)
            .getReward(bob.address, rewardsToGet);
          const claimReceipt = await claimTx.wait();

          // Extract claimed amounts from RewardPaid events
          const rewardPaidEvents =
            claimReceipt.events?.filter((e) => e.event === "RewardPaid") || [];
          expect(rewardPaidEvents.length).to.equal(1);
          const token2Event = rewardPaidEvents.find(
            (e) => e.args?.rewardToken === token2.address
          );
          ...
          expect(finalToken2Balance).to.be.closeTo(
            initialToken2Balance.add(expectedToken2Reward),
            ethers.utils.parseEther("1")
          );
          ...
          const token1ClaimTx = await farmingRewardsDistributor
            .connect(bob)
            .getReward(bob.address, token1RewardsToGet);
          const token1ClaimReceipt = await token1ClaimTx.wait();

          // Extract claimed amounts from RewardPaid events
          const token1RewardPaidEvents =
            token1ClaimReceipt.events?.filter(
              (e) => e.event === "RewardPaid"
            ) || [];
          const token1Event = token1RewardPaidEvents.find(
            (e) => e.args?.rewardToken === token1.address
          );
          ...
          expect(afterToken1ClaimBalance).to.be.closeTo(
            initialToken1Balance.add(expectedToken1Reward),
            ethers.utils.parseEther("1")
          );
        });
        ...
        it("Distributes rewards correctly between multiple users", async () => {
          ...
          await algebraVault
            .connect(alice)
            .deposit(ethers.utils.parseEther("2000"), 0, alice.address);
          await algebraVault
            .connect(bob)
            .deposit(ethers.utils.parseEther("1000"), 0, bob.address);
          ...
          await algebraVault
            .connect(alice)
            .approve(farmingRewardsDistributor.address, aliceLpBalance);
          await algebraVault
            .connect(bob)
            .approve(farmingRewardsDistributor.address, bobLpBalance);
          ...
          const [, aliceAmounts] =
            await farmingRewardsDistributor.claimableRewards(alice.address);
          const [, bobAmounts] =
            await farmingRewardsDistributor.claimableRewards(bob.address);
        ...
        it("Updates rewards on the subsequent stakes", async () => {
          ...
          await farmingRewardsDistributor
            .connect(alice)
            .stake(lpBalance.div(2), alice.address);
        ...
        it("Transfers rewards to distributor contract on withdraw", async () => {
          ...
          await farmingRewardsDistributor
            .connect(alice)
            .stake(lpBalance.sub(1000), alice.address);
        ...
        it("Distibute reward after blurred shares", async () => {
          //prepare
          // let wallet_balance_lp_before = await algebraVault.balanceOf(
          //   wallet.address
          // );
          // await algebraVault.withdraw(wallet_balance_lp_before, wallet.address);
          // await farmingRewardsDistributor.updateReward();
          // console.log(wallet_balance_lp_before);

          // await algebraVault.connect(wallet).deposit(1, 0, wallet.address);
          // let wallet_liq_balance = await algebraVault.balanceOf(wallet.address);
          // await farmingRewardsDistributor.stake(
          //   wallet_liq_balance,
          //   wallet.address
          // );
          ...
          await algebraVault
            .connect(alice)
            .deposit(ethers.utils.parseEther("10000"), 0, alice.address);
          await algebraVault
            .connect(bob)
            .deposit(ethers.utils.parseEther("10000"), 0, bob.address);
          // await token0.mint(
          //   algebraVault.address,
          //   ethers.utils.parseEther("10000")
          // );
          let bob_liq_amount = await algebraVault.balanceOf(bob.address);
          await algebraVault.connect(bob).withdraw(bob_liq_amount, bob.address);
          ...
          await algebraVault
            .connect(carol)
            .deposit(ethers.utils.parseEther("10000"), 0, carol.address);
          ...
          await algebraVault
            .connect(alice)
            .approve(farmingRewardsDistributor.address, veryLargeTokenAmount);
          await algebraVault
            .connect(carol)
            .approve(farmingRewardsDistributor.address, veryLargeTokenAmount);
          ...
          await farmingRewardsDistributor
            .connect(alice)
            .stake(alice_liq_balance, alice.address);
          await farmingRewardsDistributor
            .connect(carol)
            .stake(carol_liq_balance, carol.address);
        ...
        it("Puase/Unpause stake", async () => {
          await algebraVault
            .connect(alice)
            .deposit(ethers.utils.parseEther("1"), 0, alice.address);
          await algebraVault
            .connect(alice)
            .approve(farmingRewardsDistributor.address, veryLargeTokenAmount);
          const aliceLpBalance = await algebraVault.balanceOf(alice.address);
        ...
        it("Recover erc20", async () => {
          ...
          await expect(
            farmingRewardsDistributor
              .connect(wallet)
              .recoverERC20(token3.address, veryLargeTokenAmount)
          ).to.be.emit(farmingRewardsDistributor, "Recovered");

          await algebraVault
            .connect(alice)
            .deposit(ethers.utils.parseEther("1000"), 0, alice.address);
          await algebraVault
            .connect(alice)
            .approve(farmingRewardsDistributor.address, giantTokenAmount);
          await farmingRewardsDistributor
            .connect(alice)
            .stake(await algebraVault.balanceOf(alice.address), alice.address);

          await token2.approve(algebraEternalFarming.address, giantTokenAmount);
          await token1.approve(algebraEternalFarming.address, giantTokenAmount);
          ...
          ).to.be.emit(farmingRewardsDistributor, "RewardPaid");
        });
        it("check getUserRewardPerToken",async()=>{
          await farmingRewardsDistributor.getUserRewardPerToken(alice.address,token0.address);
        })
        it("check getUserData", async()=>{
          await farmingRewardsDistributor.getUserData(alice.address);
        })
      });
    });
    ...
    describe("Farming detach", () => {
      ...
      it("Add new token as reward", async () => {
        ...
        let nonce = await algebraEternalFarming.numOfIncentives();
        await token3.approve(algebraEternalFarming.address, totalReward);
        ...
        await farmingRewardsDistributor.addReward(token3.address);
